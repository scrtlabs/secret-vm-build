...
DOCKER_COMPOSE_HASH=$(sha256sum $SCRIPTS_DIR/config/docker-compose.yaml | cut -f1 -d' ')
ROOTFS_HASH=$(sha256sum $ROOT_DIR/sev/rootfs-prod.iso | cut -f1 -d' ')

# Compute image fingerprints and write them to the config directory.
# The VM reads these at boot to include in the dstack KMS request so
# the KMS can recompute and verify the hardware MEASUREMENT.
