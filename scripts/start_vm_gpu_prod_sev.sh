...
ROOTFS_HASH=$(sha256sum $ARTIFACTS_DIR/sev/rootfs-gpu-prod.iso | cut -f1 -d' ')
...
qemu-system-x86_64 -D ${VM_NAME}.log \
                   ...
                   -append "console=ttyS0 loglevel=7 pci=realloc,nocrs docker_compose_hash=$DOCKER_COMPOSE_HASH rootfs_hash=$ROOTFS_HASH" \
                   -enable-kvm \
                   ...
                   -drive file=$ARTIFACTS_DIR/sev/rootfs-gpu-prod.iso,if=none,id=cdrom0,format=raw,media=cdrom,readonly=on \
