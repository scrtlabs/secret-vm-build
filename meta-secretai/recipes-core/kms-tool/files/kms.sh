...
get_master_secret()
{
    ...
    local quote=$(/usr/bin/attest_tool attest $pubkey)
    if ! test_valid_hex_data "quote"; then
        return 1
    fi

    local collateral=$(/usr/bin/dcap_collateral_tool $quote |sed -n '3p')
...
sudo /usr/bin/attest_tool report | sudo tee /mnt/secure/self_report.txt > /dev/null
