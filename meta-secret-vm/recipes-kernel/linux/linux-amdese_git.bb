...
do_configure:append() {
    for config_fragment in ${WORKDIR}/*.cfg; do
        if [ -e "$config_fragment" ]; then
            echo "Applying kernel fragment: $config_fragment"
            ${S}/scripts/kconfig/merge_config.sh -m -O ${B} ${B}/.config "$config_fragment"
        fi
    done
